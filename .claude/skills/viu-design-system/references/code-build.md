...
`account_circle` (Avatar), `stat_1`/`stat_minus_1` (chevrons ↑/↓ de TimePicker), `sell` (ícono default del Badge, jul-2026) + **6 sumados sep-2026 por el portafolio** (`stacked_email`, `chat_bubble`, `emoji_language`, `star`, `download`, `arrow_back`): los cuatro primeros son los del header del portafolio (Figma `Quick Contact Icons` + botón Resume), `download` el del botón de résumé y `arrow_back` el par de `arrow_forward` (el carrusel del case study usa ←/→ y rotar por CSS era peor que el símbolo oficial) + **`send` y `menu` sumados sep-2026** (`send`: el botón de enviar del Portfolio concierge — `arrow_forward` leía como "siguiente", no como "enviar"; `menu`: el hamburger del menú móvil del portafolio; `play_arrow` + `pause`: el control de pausa del video del hero — WCAG 2.2.2 exige poder frenar cualquier movimiento automático de más de 5s; `open_in_full`: expandir ese mismo video a pantalla completa). Todos copiados del repo `google/material-design-icons` (Outlined 400), no dibujados a mano.
